Shader "Hidden/FernNPR/PostProcess/EdgeDetectionOutline"
{
    ...
    float4 EdgeDetectionFragment (PostProcessVaryings input) : SV_Target
    {
        ...
        #if _LowQuality
                float4 neighborhood = Sample4Neighborhood(uv,  _Edge_Threshold.y);
        #else
            float4 neighborhood = SampleNeighborhood(uv,  _Edge_Threshold.y);
        #endif
        float normalSampler = smoothstep(_Edge_Threshold.x, 1, dot(center.xyz, neighborhood.xyz));
        float depthSampler = smoothstep(_Edge_Threshold.z * center.w, 0.0001f * center.w, abs(center.w - neighborhood.w));
        float edge = 1 - normalSampler * depthSampler;

        return edge * _Edge_Threshold.w;

        // TODO: Use For Uber 
        float4 color = LOAD_TEXTURE2D_X(_BlitTexture, uv * _SourceSize.xy);
        color.rgb = lerp(color.rgb, _Edge_Color, edge * _Edge_Threshold.w);
        return color;
    }
    ENDHLSL
